...
if [ -z "$TARGET_HOME" ]; then
  ...
  echo
  echo "   Options:"
  echo "   1. MOST LIKELY FIX — the app gates its launcher component behind an"
  echo "      in-app setting that adb cannot reach. Turn it on with the remote,"
  echo "      then re-run this script:"
  echo "        Fully Kiosk  ->  Settings -> Kiosk Mode -> ENABLE Kiosk Mode"
  echo "        (it then offers to set itself as Home App - accept)"
  echo "      Set the Start URL while you are in there too."
